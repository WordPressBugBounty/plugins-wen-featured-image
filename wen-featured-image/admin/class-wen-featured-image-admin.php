...
class Wen_Featured_Image_Admin
{
    ...
    function plugin_section_column_text_callback()
    {
        ...
        echo sprintf(
            wp_kses_post(
                // Translators: %s is a Featured Image title 
                __('Enable / Disable %s column in listings.', 'wen-featured-image')
            ),
            '<strong>' . esc_html__('Featured Image', 'wen-featured-image') . '</strong>'
        );
    }
    ...
    function plugin_section_message_text_callback()
    {
        ...
        echo sprintf(
            wp_kses_post(
                // Translators: Message will be displayed in %s featured image metabox
                __('These messages will be displayed in the %s metabox.', 'wen-featured-image')
            ),
            '<strong>' . esc_html__('Featured Image', 'wen-featured-image') . '</strong>'
        );
    }
    ...
    function plugin_section_required_text_callback()
    {
        ...
        echo sprintf(
            wp_kses_post(
                // Translators: Make %s featured image Required for. 
                __('Make %s required.', 'wen-featured-image')
            ),
            '<strong>' . esc_html__('Featured Image', 'wen-featured-image') . '</strong>'
        );
    }
...
<?php
    ...
    function posts_column_content($column, $post_id)
    {
        ...
        if ('wfi_image' == $column) {
            ...
            echo wp_kses_post($this->get_image_block_html($post_thumbnail_id));
            echo '</div>';
        } // end if wfi_column
    ...
    function wfi_admin_notices()
    {
        ...
        if ('no' == get_transient('wfi_req_check')) {
            ...
            echo wp_kses_post($this->options['required_message']);
            echo '</strong></p></div>';
    ...
    function wfi_table_filtering()
    {
        ...
        echo '<option value="" >' . esc_html__('Show All', 'wen-featured-image') . '</option>';
        echo '<option value="yes" ' . selected($selected_now, 'yes', false) . '>' . esc_html__('Featured Image', 'wen-featured-image') . '</option>';
        echo '<option value="no" ' . selected($selected_now, 'no', false) . '>' . esc_html__('No Featured Image', 'wen-featured-image') . '</option>';
        echo '</select>';
    }
